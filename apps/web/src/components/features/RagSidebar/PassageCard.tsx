...
function extractHighlightTerms(summary: string): string[] {
  const words = summary
    .toLowerCase()
    .split(/[^a-z0-9]+/)
...
function renderHighlightedExcerpt(excerpt: string, summary: string) {
  const terms = extractHighlightTerms(summary)
  if (terms.length === 0) {
  ...
  return (
    <>
      {parts.map((part, index) => {
        ...
        return (
          <span key={`${part}-${index}`} className="font-semibold text-[#2D5016]">
            {part}
          </span>
        )
      })}
...
export function PassageCard({ result, sourceColor, isSummaryLoading }: PassageCardProps) {
  return (
    <article
      draggable
      onDragStart={(event) => {
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-2">
        <div className="min-w-0 flex-1">
          <p className="line-clamp-1 text-xs font-semibold tracking-[0.01em] text-[#1C1917]">
            {result.sourceName}
          </p>
          <p className="mt-1 text-[11px] text-[#78716C]">{result.locationLabel}</p>
        </div>
      ...
      <div className="mt-3 min-w-0 min-h-[3.1rem]">
        <div
          className={`transition-all duration-200 ${
            result.summary ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0'
          }`}
        >
          {result.summary ? (
            <div className="min-w-0 max-w-full overflow-hidden rounded-md border border-[#EAE2D6] bg-[#F8F4EC]/90 px-2.5 py-2">
              ...
              <p className="mt-1 line-clamp-3 max-w-full text-sm font-medium leading-5 text-[#1C1917] break-words [overflow-wrap:anywhere]">
                {result.summary}
              </p>
            </div>
          ) : null}
      ...
      <div className="mt-3 min-w-0 max-w-full overflow-hidden rounded-md border border-[#EFE7DB] bg-[#FCFAF5] px-2.5 py-2.5">
        ...
        <p className="mt-1 line-clamp-5 max-w-full text-sm leading-6 text-[#292524] break-words [overflow-wrap:anywhere]">
          {renderHighlightedExcerpt(result.excerpt, result.summary)}
        </p>
      </div>
    </article>
  )
